...
- (NSString*)labelForElementAtRow:(NSUInteger)row{
    if(_dataSource && [_dataSource respondsToSelector:@selector(chartView:labelForElementAtRow:)]){
        NSString* label = [_dataSource chartView:self labelForElementAtRow:row];
        return label;
    }else{
        NSAssert(_dataSource, @"Data source is not set.");
        NSAssert([_dataSource respondsToSelector:@selector(chartView:labelForElementAtRow:)], @"chartView:labelForElementAtRow: not implemented.");
        return @"";
    }
}
...
-(void)setGradientStartColor:(UIColor *)gradientStartColor{
    _internalChartView.gradientStartColor = gradientStartColor;
}

-(void)setGradientEndColor:(UIColor *)gradientEndColor{
    _internalChartView.gradientEndColor = gradientEndColor;
}
